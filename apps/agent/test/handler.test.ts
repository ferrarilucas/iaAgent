...
describe("processMessage", () => {
  it("faz bootstrap no primeiro contato, marca lido, mostra typing e registra via agent", async () => {
    ...
    const incoming: IncomingMessage = { messageId: "M1", remoteJid: "5511@s.whatsapp.net", fromNumber: "5511", fromMe: false, pushName: "Lucas", kind: "texto", text: "gastei 50 no almoco" };
    ...
    expect(markAsRead).toHaveBeenCalledWith({ remoteJid: "5511@s.whatsapp.net", id: "M1", fromMe: false });
    expect(setTyping).toHaveBeenCalledWith("5511");
  });
  ...
  it("ignora mensagem duplicada (idempotencia)", async () => {
    ...
    const markAsRead = vi.fn(async () => {});
    const setTyping = vi.fn(async () => {});
    const incoming: IncomingMessage = { messageId: "DUP", remoteJid: "5511@s.whatsapp.net", fromNumber: "5511", fromMe: false, kind: "texto", text: "oi" };
    await processMessage({ db: t.db, runAgent, sendText, markAsRead, setTyping }, incoming);
    await processMessage({ db: t.db, runAgent, sendText, markAsRead, setTyping }, incoming);
  ...
  it("envia fallback e nao rejeita quando runAgent falha", async () => {
    ...
    const markAsRead = vi.fn(async () => {});
    const setTyping = vi.fn(async () => {});
    const incoming: IncomingMessage = { messageId: "FAIL1", remoteJid: "5511@s.whatsapp.net", fromNumber: "5511", fromMe: false, kind: "texto", text: "gastei 50 no almoco" };

    await expect(processMessage({ db: t.db, runAgent, sendText, markAsRead, setTyping }, incoming)).resolves.toBeUndefined();
